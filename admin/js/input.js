	...
	"fnInit": function (oSettings, nPaging, fnCallbackDraw) {
		...
		jQuery(nFirst).click(function () {
			...
			if (iCurrentPage != 1) {
				...
				jQuery(nFirst).addClass('disabled');
				jQuery(nPrevious).addClass('disabled');
				jQuery(nNext).removeClass('disabled');
				jQuery(nLast).removeClass('disabled');
			}
		});
		...
		jQuery(nNext).click(function () {
			...
			if (iCurrentPage != Math.ceil((oSettings.fnRecordsDisplay() / oSettings._iDisplayLength))) {
				...
				if (iCurrentPage == (Math.ceil((oSettings.fnRecordsDisplay() - 1) / oSettings._iDisplayLength) - 1)) {
					jQuery(nNext).addClass('disabled');
					jQuery(nLast).addClass('disabled');
				}
				jQuery(nFirst).removeClass('disabled');
				jQuery(nPrevious).removeClass('disabled');
			}
		});
